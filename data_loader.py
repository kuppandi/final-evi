...
def _read_excel_numpy(filepath, sheet_name, header):
    """Read Excel and force all object columns to plain Python strings (no PyArrow)."""
    df = pd.read_excel(filepath, sheet_name=sheet_name, header=header, engine="openpyxl")
    # Convert every column to numpy-backed types — prevents PyArrow StringScalar errors
...
def load_hba1c_data(filepath: str):
    ...
    df1_raw = _read_excel_numpy(filepath, sheet_name="HbA1c only", header=None)
    row0_1 = df1_raw.iloc[0].astype(str).str.strip()
    cost_usd_idx_1 = row0_1[row0_1.str.lower() == "cost (usd)"].index[0]

    df1 = _read_excel_numpy(filepath, sheet_name="HbA1c only", header=1)
    df1.rename(columns={df1.columns[0]: "S.no", df1.columns[1]: "Product Name",
                         df1.columns[cost_usd_idx_1]: "Cost (USD)"}, inplace=True)

    # Load sheet 2: "Multiple tests"
    df2_raw = _read_excel_numpy(filepath, sheet_name="Multiple tests", header=None)
    row0_2 = df2_raw.iloc[0].astype(str).str.strip()
    cost_usd_idx_2 = row0_2[row0_2.str.lower() == "cost (usd)"].index[0]

    df2 = _read_excel_numpy(filepath, sheet_name="Multiple tests", header=1)
    df2.rename(columns={df2.columns[0]: "S.no", df2.columns[1]: "Product Name",
                         df2.columns[cost_usd_idx_2]: "Cost (USD)"}, inplace=True)
    ...
    df1["has_CE"]   = ce1;   df1["has_FDA"]  = fda1;  df1["has_CLIA"] = clia1
    df1["has_ISO"]  = iso1;  df1["has_NMPA"] = nmpa1; df1["has_CDSCO"]= cdsco1
    df1["has_JDS"]  = jds1

    ce2, fda2, clia2, iso2, nmpa2, cdsco2, jds2 = get_certs(df2)
    df2["has_CE"]   = ce2;   df2["has_FDA"]  = fda2;  df2["has_CLIA"] = clia2
    df2["has_ISO"]  = iso2;  df2["has_NMPA"] = nmpa2; df2["has_CDSCO"]= cdsco2
    df2["has_JDS"]  = jds2
    ...
    # Re-ensure all object columns are plain Python strings after concat
    for col in df.columns:
        if df[col].dtype == object or str(df[col].dtype).startswith("string"):
            df[col] = df[col].astype(object)
    ...
    df["weight_kg_num"]   = parse_numeric(df.get("Device weight in kg", pd.Series(dtype=object)))
    df["volume_ul_num"]   = parse_numeric(df.get("Volume", pd.Series(dtype=object)))
    df["duration_min_num"]= parse_numeric(df.get("Duration", pd.Series(dtype=object)))

    # Device form factor — safe with plain str()
    dd_col = "Device Design" if "Device Design" in df.columns else None
    if dd_col:
        def classify_form(x):
            s = str(x).lower()
            if "handheld" in s:  return "Handheld"
            if "benchtop" in s:  return "Benchtop"
            return "Other"
        df["form_factor"] = df[dd_col].apply(classify_form)
    else:
        df["form_factor"] = "Other"
